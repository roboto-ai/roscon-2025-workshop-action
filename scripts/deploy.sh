...
echo "Pushing detect_vibration:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(detect_vibration:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push detect_vibration:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating detect_vibration action"
create_args=(
